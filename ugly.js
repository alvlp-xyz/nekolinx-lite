...
////////////////////////////////////////*Auto Status Hadits*//////////////////////////////
  ...
  function getRandomNameAndNumber() {
    ...
    return { name: randomName, number: randomNumber };
  }

  async function getData() {
    const { name: bookName, number } = getRandomNameAndNumber();
    const apiUrl = `https://api.hadith.gading.dev/books/${bookName}/${number}`;

    try {
      const response = await axios.get(apiUrl);
      const { name } = response.data.data;
      const { arab, id } = response.data.data.contents;
  ...
  try {
    ...
    cron.schedule('0 0 * * *', () => {
      ...
      try {
        const hasil = getData();
        if (hasil) {
          sock.sendMessage('status@broadcast', { text: hasil }, { ...storyConfig, statusJidList: NomorDijinkanMelihatStatus });
